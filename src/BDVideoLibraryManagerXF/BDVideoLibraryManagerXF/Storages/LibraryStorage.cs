...
public static class LibraryStorage
{
	...
	public static async Task<bool> TryCopy(string serverName, string remotePath, string UserName, string Password, bool DoCopy = false)
	{
		...
		if (serverName.Equals("test.example", StringComparison.InvariantCultureIgnoreCase) || serverName.EndsWith(".test", StringComparison.InvariantCultureIgnoreCase))
		{
			try
			{
				var folder2 = System.IO.Directory.CreateDirectory(PathCsvClosed);
				folder2.Delete(true);
				folder2.Create();
				...
				for (int i = 1; i <= 3; i++)
					await webClient.DownloadFileTaskAsync(
						new Uri($"https://raw.githubusercontent.com/kurema/BDVideoLibraryManager/master/res/csv/{i:000}.csv"),
